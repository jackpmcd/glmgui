...
function(workspace) {
  ...
  cb <<- gcombobox(l, selected = 0, editable=TRUE, container=group_set_g1_g,handler=function(h,...){
    ...
    selectedValue<<- get_nml_value(eg_nml,arg_name = svalue(h$obj))
    if(length(List_parameter)!=0){
     if(List_parameter[[1]]==svalue(cb)) {
      ...
      else if(length(selectedValue)>1){
        for (k in 1:length(selectedValue)) {
          if(k==1){
            textarg<-selectedValue[k]
          }
          else{textarg<-paste(textarg, selectedValue[k], sep =",")}
          
        }
        svalue(value)<<-textarg
      }
      else{
        if(is.numeric(selectedValue)){print("numeric")}
        else if ( is.character(selectedValue)){print("character")}
      ...
      svalue(value)<<-textarg
      if(is.numeric(selectedValue)){print("numeric (multiple)")} 
      else if ( is.character(selectedValue)){print("character")}
      selected<<- svalue(h$obj) #ENDNEW
    }
    else{
      if(is.numeric(selectedValue)){print("numeric")}
      else if ( is.character(selectedValue)){print("character")}
  ...
  button_set_parameter<<-gbutton("Set", container = group_set_g2_g,handler = function(h, ...) {
    ...
    if(svalue(value)!=""){
      ...
      if(grepl(";",svalue(value)) && length(List_parameter)==0){
        ...
        print("delete Interval and set value")
        ### Delets Interval for single value
        List_parameter <<- list()
        List_values <<- list()
        svalue(label_nml_range)<<-""

        
        if(is.numeric(selectedValue)){
          selectedValue <- as.numeric(svalue(value))
          print(selectedValue)
          
          eg_nml <- set_nml(eg_nml,arg_name =  gewaehlt ,arg_val = selectedValue)
          nml_file = file.path(workspace , "glm3.nml")
          write_nml(eg_nml, file = nml_file)
          
        }
        else if(is.character(selectedValue)){
          selectedValue <- as.character(svalue(value))
          print(selectedValue)
        ...
        if(is.numeric(selectedValue)){
          selectedValue <- as.numeric(svalue(value))
          eg_nml <-read_nml(nml_file)
          eg_nml <- set_nml(eg_nml,arg_name =  selected ,arg_val = selectedValue)
          nml_file = file.path(workspace , "glm3.nml")
          write_nml(eg_nml, file = nml_file)
        ...
        if(is.numeric(selectedValue)){
          selectedValue <- as.numeric(strsplit(svalue(value), ',')[[1]])
          eg_nml <-read_nml(nml_file)
          eg_nml <- set_nml(eg_nml,arg_name =  selected ,arg_val = selectedValue)
