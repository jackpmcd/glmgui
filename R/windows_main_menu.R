windows_main_menu <-
function(version){
  win_main <- gwindow("General Lake Model Toolbox", width = 800, height = 600)
  content_project <- ggroup(horizontal = FALSE, use.scrollwindow = TRUE, container = win_main)
  ...
  button_workspace <<- gbutton("Open Project", container = content_project_workspace_1, handler = function(h, ...) {
    ...
    if(dir!=""){
      ...
      svalue(label_workspace)<- dir
      if(file.exists(dir)){
        show_message("Found glm3.nml file. \nCreating project.")
  ...
  gbutton("GLM Version", container = content_project_build2, handler=function(h,...) {
    ...
    gmessage(paste("The current version of GLM is", glm_version, sep=" "), title = 'GLM Version')})
